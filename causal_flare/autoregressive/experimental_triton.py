...
def _phase3_bwd_workspace_tensor(
    name: str,
    shape: tuple[int, ...],
    *,
    device: torch.device,
...
def phase3_dense_output_backward_reference(
    C: torch.Tensor,
    W: torch.Tensor,
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    if C.ndim != 4 or W.ndim != 4 or V.ndim != 4 or log_alpha.ndim != 3 or grad_Y.ndim != 4:
        raise ValueError(
            "phase3_dense_output_backward_reference expects "
    ...
    if C_CHUNK == 0:
        raise NotImplementedError("phase3_dense_output_backward_reference does not support C==0.")
    _require_supported_md(M, D, where="phase3_dense_output_backward_reference")
    _require_nonpositive_log_alpha(log_alpha, where="phase3_dense_output_backward_reference")
...
@triton.jit
def phase1_local_chunk_end_state_bwd_dw_chunk_kernel(
...
@triton.jit
def phase1_local_chunk_end_state_bwd_dv_b_chunk_kernel(
    grad_s_local_end_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
class Phase1LocalChunkEndStateTrition(torch.autograd.Function):
    ...
    def backward(ctx, grad_s_local_end: torch.Tensor):
        ...
        phase1_local_chunk_end_state_bwd_dw_chunk_kernel[grid_dw](
            grad_s_md,
            V,
            log_alpha,
            d_W,
        ...
        phase1_local_chunk_end_state_bwd_dv_b_chunk_kernel[grid_dv](
            grad_s_md,
            W,
            V,
            log_alpha,
...
@triton.jit
def phase3_dense_output_bwd_a1_kernel(
    C_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
@triton.jit
def phase3_dense_output_bwd_a2_kernel(
    C_ptr,
    W_ptr,
    log_alpha_ptr,
    dK_ptr,
...
@triton.jit
def phase3_dense_output_bwd_off_dc_kernel(
    C_ptr,
    grad_y_ptr,
    log_alpha_ptr,
    S0_ptr,
...
@triton.jit
def phase3_dense_output_bwd_off_ds0_kernel(
    C_ptr,
    grad_y_ptr,
    log_alpha_ptr,
    out_dS0_ptr,
...
@triton.jit
def phase3_dense_output_bwd_logalpha_diag_kernel(
    Q_ptr,
    out_dlog_diag_ptr,
    bh_size,
    nc_size,
...
@triton.jit
def phase3_dense_output_bwd_logalpha_off_kernel(
    y_off_ptr,
    grad_y_ptr,
    out_dlog_off_ptr,
    bh_size,
...
def phase3_dense_output_backward_kernel_profile(
    ...
    This mirrors `Phase3DenseOutputTrition.backward` without autograd plumbing so
    kernel-level bottlenecks can be tuned quickly.
    """
    if C.ndim != 4 or W.ndim != 4 or V.ndim != 4 or log_alpha.ndim != 3 or grad_out.ndim != 4:
        raise ValueError(
            "phase3_dense_output_backward_kernel_profile expects "
    ...
    _require_supported_md(M, D, where="phase3_dense_output_backward_kernel_profile")
    _require_chunk_size_multiple_of_16(C_CHUNK, where="phase3_dense_output_backward_kernel_profile")
    _require_nonpositive_log_alpha(log_alpha, where="phase3_dense_output_backward_kernel_profile")
    if not C.is_cuda:
        raise NotImplementedError("phase3_dense_output_backward_kernel_profile requires CUDA tensors.")
    _ensure_triton_allocator()
    ...
    def _run_a1():
        ...
        phase3_dense_output_bwd_a1_kernel[grid_a1](
            C,
            W,
            V,
            log_alpha,
    ...
    def _run_a2():
        ...
        phase3_dense_output_bwd_a2_kernel[grid_a2](
            C,
            W,
            log_alpha,
            dK,
    ...
    def _run_diag():
        grid_diag = (BH * NC,)
        phase3_dense_output_bwd_logalpha_diag_kernel[grid_diag](
            Q,
            dlog_diag,
            BH,
            NC,
    ...
    if has_s0:
        ...
        def _run_off_dc():
            ...
            phase3_dense_output_bwd_off_dc_kernel[grid_off_dc](
                C,
                grad_out_c,
                log_alpha,
                S0_md,
        ...
        def _run_off_ds0():
            ...
            phase3_dense_output_bwd_off_ds0_kernel[grid_off_ds0](
                C,
                grad_out_c,
                log_alpha,
                dS0,
        ...
        def _run_off_log():
            grid_off_log = (BH * NC,)
            phase3_dense_output_bwd_logalpha_off_kernel[grid_off_log](
                y_off_saved,
                grad_out_c,
                dlog_diag,
                BH,
...
class Phase3DenseOutputTrition(torch.autograd.Function):
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        if M % BLOCK_M != 0 or D % BLOCK_D != 0:
            raise NotImplementedError(
                f"Phase3DenseOutputTrition.backward requires M%BLOCK_M==0 and D%BLOCK_D==0; "
        ...
        dV = torch.empty_like(V)
        if n_d_tiles == 1:
            dK = _phase3_bwd_workspace_tensor(
                "dK",
            ...
            dK_partials = _phase3_bwd_workspace_tensor(
                "dK_partials",
        ...
        R_buf = _phase3_bwd_workspace_tensor(
            "R_buf",
            (BH, NC, C_CHUNK, C_CHUNK),
            device=C.device,
            dtype=torch.float32,
        ...
        dC_diag = torch.empty_like(C)
        dW = torch.empty_like(W)
        Q = _phase3_bwd_workspace_tensor(
            "Q",
            (BH, NC, C_CHUNK, C_CHUNK),
            device=C.device,
            dtype=torch.float32,
        ...
        dlog_diag = _phase3_bwd_workspace_tensor(
            "dlog_diag",
            (BH, NC, C_CHUNK),
            device=C.device,
            dtype=torch.float32,
...
@triton.jit
def phase2_prefix_scan_bwd_dense_kernel(
    grad_prefix_ptr,
    grad_final_ptr,
    chunk_start_ptr,
    log_alpha_chunk_ptr,
...
class Phase2PrefixScanTrition(torch.autograd.Function):
    ...
    def forward(
        ...
        _require_nc_multiple_of_16(NC, where="Phase2PrefixScanTrition.forward")
        ...
        if compute_dtype not in (torch.float16, torch.bfloat16, torch.float32):
            raise NotImplementedError(
                "Phase2PrefixScanTrition.forward supports compute_dtype in "
        ...
        phase2_prefix_scan_fwd_kernel[grid_fwd](
            S_local_end_in,
            log_alpha_chunk_f,
            initial_state_f,
            chunk_start,
    ...
    def backward(ctx, grad_chunk_start: torch.Tensor, grad_final_state: torch.Tensor):
        ...
        _require_nc_multiple_of_16(NC, where="Phase2PrefixScanTrition.backward")
        ...
        phase2_prefix_scan_bwd_dense_kernel[grid_bwd](
            grad_chunk_start_f,
            grad_final_state_f,
            chunk_start,
            log_alpha_chunk,
...
def phase2_prefix_scan_triton_outline(
    ...
    return Phase2PrefixScanTrition.apply(S_local_end, log_alpha_chunk, initial_state, compute_dtype)
...
def phase123_full_parallel_scan_trition(
    ...
    S0_chunk, S1_chunk = phase2_prefix_scan_triton_outline(
        S_local_end,
        log_alpha_per_chunk,
        init_flat,
