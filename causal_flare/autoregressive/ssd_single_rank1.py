...
def _ssd_single_rank1_bwd_workspace_tensor(
    name: str,
    shape: tuple[int, ...],
    *,
    device: torch.device,
...
def _ssd_single_rank1_prepare_unchunked_inputs(
    C: torch.Tensor,
    W: torch.Tensor,
    V: torch.Tensor,
...
def _ssd_single_rank1_restore_output_layout(
...
def ssd_single_rank1_pytorch(
    ...
    """Chunked single-state PyTorch reference entrypoint.
    ...
    if V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
            "ssd_single_rank1_pytorch expects V=[B,N,H,D], log_alpha=[B,N,H]. "
    ...
    if N == 0:
        raise NotImplementedError("ssd_single_rank1_pytorch does not support N==0.")
    if CHUNK_SIZE <= 0:
        raise ValueError(f"CHUNK_SIZE must be positive; got {CHUNK_SIZE}.")
    _require_nonpositive_log_alpha(log_alpha, where="ssd_single_rank1_pytorch")
    ...
    V_chunk = (
        V.reshape(B, NC, CHUNK_SIZE, H, D)
        .permute(0, 3, 1, 2, 4)
        .contiguous()
        .reshape(BH, NC, CHUNK_SIZE, D)
    )
    log_alpha_chunk = (
        log_alpha.reshape(B, NC, CHUNK_SIZE, H)
        .permute(0, 3, 1, 2)
        .contiguous()
        .reshape(BH, NC, CHUNK_SIZE)
    )
    ...
    y = (
        y_chunk.reshape(B, H, NC, CHUNK_SIZE, D)
        .permute(0, 2, 3, 1, 4)
        .reshape(B, NC * CHUNK_SIZE, H, D)
    )[:, :N]
    final_state = S1_chunk.reshape(B, H, D)
    return y, final_state
...
def ssd_single_rank1_token_loop_oracle(
    ...
    """Unchunked token-loop oracle for single-state recurrence.
    ...
    - V: `[B, N, H, D]`
    - log_alpha: `[B, N, H]`
    - initial_state: optional `[BH,D]` or `[B,H,D]`
...
def ssd_single_rank1_chunk_end_state_reference(
    ...
    """Phase-1 reference: per-chunk local end-state for single-state recurrence.
    ...
    - S_local_end: `[BH, NC, D]`
      End S for each chunk assuming zero chunk start:
        S_{t+1} = alpha_t * S_t + V_t, with S_0 = 0
        where `alpha_t = exp(log_alpha_t)`.
      and `S_local_end = S_C`.

    Closed form:
      S_local_end = sum_{j=0..C-1} (prod_{u=j+1..C-1} alpha_u) * V_j

    Vectorized implementation:
      Let
        r_j = prod_{u=j+1..C-1} alpha_u
      then:
        S_local_end[D] = sum_j r_j * V_j[D]
    """
    if V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
    ...
    if log_alpha.shape != (BH, NC, C):
        raise ValueError(
            "log_alpha must match [BH,NC,C] from V. "
            f"Got log_alpha={tuple(log_alpha.shape)}, V={tuple(V.shape)}."
        )
    if V.device != log_alpha.device:
        raise ValueError("V and log_alpha must be on the same device.")
    if V.dtype != log_alpha.dtype:
    ...
    S_local_end = torch.sum(r.unsqueeze(-1) * V_f, dim=-2)
    return S_local_end
...
def ssd_single_rank1_prefix_scan_reference(
    ...
    Inputs:
    - S_local_end: `[BH, NC, D]` (or `[BHG, NC, D]` if grouping is folded outside)
    - alpha_chunk: `[BH, NC]` (or `[BHG, NC]`)
    - initial_state: `[BH, D]` (or `[BHG, D]`)
    ...
    BH, NC, state_dim = S_local_end.shape
    ...
    if initial_state.shape != (BH, state_dim):
        raise ValueError(
            "initial_state must match [BH,D] from S_local_end. "
...
@triton.jit
def ssd_single_rank1_chunk_end_state_fwd_kernel(
    W_ptr,
    V_ptr,
    log_alpha_ptr,
    out_s_local_end_ptr,
...
@triton.jit
def ssd_single_rank1_chunk_end_state_bwd_dw_chunk_kernel(
...
@triton.jit
def ssd_single_rank1_chunk_end_state_bwd_dv_b_chunk_kernel(
    grad_s_local_end_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
@triton.jit
def ssd_single_rank1_chunk_end_state_bwd_dr_kernel(
    b_vals_ptr,
    log_alpha_ptr,
    out_dlog_alpha_ptr,
    bh_size,
...
def _ssd_single_rank1_chunk_end_state_forward_impl(
    W: torch.Tensor,
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    BLOCK_T: int = 32,
    ...
    if W.ndim != 4 or V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
            "_ssd_single_rank1_chunk_end_state_forward_impl expects "
    ...
    M = W.shape[-1]
    D = V.shape[-1]
    _require_supported_md(M, D, where="_ssd_single_rank1_chunk_end_state_forward_impl")
    _require_chunk_size_multiple_of_16(C, where="_ssd_single_rank1_chunk_end_state_forward_impl")
    BLOCK_T = _select_largest_block_size(
        C,
        _SUPPORTED_BLOCK_T_VALUES,
        where="_ssd_single_rank1_chunk_end_state_forward_impl",
        label="C",
    )
    _require_nc_descriptor_width(NC, where="_ssd_single_rank1_chunk_end_state_forward_impl")
    _require_phase1_block_t(BLOCK_T, C, where="_ssd_single_rank1_chunk_end_state_forward_impl")
    _require_nonpositive_log_alpha(log_alpha, where="_ssd_single_rank1_chunk_end_state_forward_impl")
    if not W.is_cuda:
            raise NotImplementedError("_ssd_single_rank1_chunk_end_state_forward_impl requires CUDA tensors.")
    ...
    ssd_single_rank1_chunk_end_state_fwd_kernel[grid](
        W,
        V,
        log_alpha,
        s_local_end_md,
...
@triton.jit
def ssd_single_rank1_prefix_scan_fwd_kernel(
    s_local_ptr,
    log_alpha_chunk_ptr,
    init_ptr,
    out_prefix_ptr,
...
@triton.jit
def ssd_single_rank1_prefix_scan_bwd_dense_kernel(
    grad_prefix_ptr,
    grad_final_ptr,
    chunk_start_ptr,
    log_alpha_chunk_ptr,
...
def _ssd_single_rank1_prefix_scan_forward_impl(
    S_local_end: torch.Tensor,
    ...
    if S_local_end.ndim != 3 or log_alpha_chunk.ndim != 2 or initial_state.ndim != 2:
        raise ValueError(
            "_ssd_single_rank1_prefix_scan_forward_impl expects "
    ...
    if NC == 0:
        raise NotImplementedError("_ssd_single_rank1_prefix_scan_forward_impl does not support NC==0.")
    _require_nc_descriptor_width(NC, where="_ssd_single_rank1_prefix_scan_forward_impl")
    _require_nc_multiple_of_16(NC, where="_ssd_single_rank1_prefix_scan_forward_impl")
    if not S_local_end.is_cuda:
        raise NotImplementedError("_ssd_single_rank1_prefix_scan_forward_impl requires CUDA tensors.")
    _ensure_triton_allocator()
    ...
    if compute_dtype not in (torch.float16, torch.bfloat16, torch.float32):
        raise NotImplementedError(
            "_ssd_single_rank1_prefix_scan_forward_impl supports compute_dtype in "
    ...
    block_nc = _select_largest_block_size(
        NC,
        (128, 64, 32, 16),
        where="_ssd_single_rank1_prefix_scan_forward_impl",
        label="NC",
    )
    grid_fwd = lambda META: (BH,)
    ssd_single_rank1_prefix_scan_fwd_kernel[grid_fwd](
        S_local_end_in,
        log_alpha_chunk_f,
        initial_state_f,
        chunk_start,
...
def ssd_single_rank1_dense_output_reference(
    ...
    """Phase-3 forward reference for single-state chunk replay.

    Forward equation per `(BH, NC)` lane:
      y_t = sum_{tau<=t} [prod_{u=tau+1..t} alpha_u] * V_tau
            + [prod_{u=0..t} alpha_u] * S0
      where alpha_u = exp(log_alpha_u), and all states are vectors in `R^D`.
    """
    if V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
    ...
    if S0 is None:
        ...
        if S0.ndim == 2 and S0.shape == (BH, D):
            S0_vec = S0[:, None, :].expand(BH, NC, D).to(torch.float32)
        elif S0.ndim == 3 and S0.shape == (BH, NC, D):
            S0_vec = S0.to(torch.float32)
        else:
            raise ValueError(
                "S0 must be [BH,D] or [BH,NC,D] matching V/log_alpha. "
                f"Got S0={tuple(S0.shape)} with expected BH={BH}, NC={NC}, D={D}."
            )
        if S0.device != V.device:
            raise ValueError("S0 must be on the same device as V/log_alpha.")

    V_f = V.float()
    log_alpha_f = log_alpha.float()

    c_idx = torch.arange(C_CHUNK, device=V.device)
    ...
    log_delta_l2 = torch.where(
        tril,
        log_delta_l2,
        torch.zeros((BH, NC, C_CHUNK, C_CHUNK), device=V.device, dtype=torch.float32),
    )
    L = torch.where(
        tril,
...
def ssd_single_rank1_dense_output_backward_reference(
    ...
    """Phase-3 backward reference for the single-state dense replay.

    Returns:
    - dV: `[BH, NC, C, D]`
    - dlog_alpha: `[BH, NC, C]`
    - dS0: `[BH, NC, D]` if `S0` is provided, else `None`
    """
    ...
    if S0 is None:
        ...
        if S0.device != V.device:
            raise ValueError("S0 must be on the same device as V/log_alpha/grad_Y.")
        return_s0_grad = True
    ...
    # Main path: Y_diag = L @ V
    dV = L.mT @ grad_Y_f
    dL = grad_Y_f @ V_f.mT

    # dlog_alpha from carry path.
    Q = dL * L
    ...
    # Off-path via Y_off = p * S0.
    dlog_alpha_off_src = (grad_Y_f * S0_vec.unsqueeze(-2)).sum(dim=-1) * p
    ...
    dS0_out = (grad_Y_f * p.unsqueeze(-1)).sum(dim=-2).to(V.dtype) if return_s0_grad else None
    return dV_out, dlog_alpha_out, dS0_out


@triton.jit
def ssd_single_rank1_dense_output_fwd_kernel(
    C_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_a1_kernel(
    C_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_a2_kernel(
    C_ptr,
    W_ptr,
    log_alpha_ptr,
    dK_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_fused_a1a2_kernel(
    C_ptr,
    W_ptr,
    V_ptr,
    log_alpha_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_fused_off_kernel(
    C_ptr,
    grad_y_ptr,
    log_alpha_ptr,
    S0_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_off_dc_kernel(
    C_ptr,
    grad_y_ptr,
    log_alpha_ptr,
    S0_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_off_ds0_kernel(
    C_ptr,
    grad_y_ptr,
    log_alpha_ptr,
    out_dS0_ptr,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_logalpha_diag_kernel(
    Q_ptr,
    out_dlog_diag_ptr,
    bh_size,
    nc_size,
...
@triton.jit
def ssd_single_rank1_dense_output_bwd_logalpha_off_kernel(
    y_off_ptr,
    grad_y_ptr,
    out_dlog_off_ptr,
    bh_size,
...
def _ssd_single_rank1_dense_output_forward_impl(
    C: torch.Tensor,
    W: torch.Tensor,
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    if C.ndim != 4 or W.ndim != 4 or V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
            "_ssd_single_rank1_dense_output_forward_impl expects C=[BH,NC,C,M], "
            "W=[BH,NC,C,M], V=[BH,NC,C,D], log_alpha=[BH,NC,C]. "
    ...
    _require_supported_md(M, D, where="_ssd_single_rank1_dense_output_forward_impl")
    _require_chunk_size_multiple_of_16(C_CHUNK, where="_ssd_single_rank1_dense_output_forward_impl")
    _require_nonpositive_log_alpha(log_alpha, where="_ssd_single_rank1_dense_output_forward_impl")
    if not C.is_cuda:
        raise NotImplementedError("_ssd_single_rank1_dense_output_forward_impl requires CUDA tensors.")
    _ensure_triton_allocator()
    ...
    BLOCK_M = _select_largest_block_size(
        M,
        _SUPPORTED_BLOCK_X_VALUES,
        where="_ssd_single_rank1_dense_output_forward_impl",
        label="M",
    )
    BLOCK_D = _select_largest_block_size(
        D,
    ...
    ssd_single_rank1_dense_output_fwd_kernel[grid](
        C,
        W,
        V,
        log_alpha,
...
def _ssd_single_rank1_dense_output_backward_impl(
    C: torch.Tensor,
    W: torch.Tensor,
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    *,
    ...
    BH, NC, C_CHUNK, M = C.shape
    D = V.shape[-1]
    _require_supported_md(M, D, where="_ssd_single_rank1_dense_output_backward_impl")
    ...
    _require_chunk_size_multiple_of_16(C_CHUNK, where="_ssd_single_rank1_dense_output_backward_impl")
    if not C.is_cuda:
        raise NotImplementedError("_ssd_single_rank1_dense_output_backward_impl requires CUDA tensors.")
    _ensure_triton_allocator()
    ...
    if M % BLOCK_M != 0 or D % BLOCK_D != 0:
        raise NotImplementedError(
            f"_ssd_single_rank1_dense_output_backward_impl requires M%BLOCK_M==0 and D%BLOCK_D==0; "
    ...
    dlog_diag = _ssd_single_rank1_bwd_workspace_tensor(
        "dlog_diag",
        (BH, NC, C_CHUNK),
        device=C.device,
        dtype=torch.float32,
    ...
    ssd_single_rank1_dense_output_bwd_fused_a1a2_kernel[grid_fused](
        C,
        W,
        V,
        log_alpha,
    ...
    if has_s0:
        ...
        ssd_single_rank1_dense_output_bwd_fused_off_kernel[grid_off](
            C,
            grad_out_c,
            log_alpha,
            S0_md,
...
def ssd_single_rank1_dense_output_backward_kernel_profile(
    C: torch.Tensor,
    W: torch.Tensor,
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    if C.ndim != 4 or W.ndim != 4 or V.ndim != 4 or log_alpha.ndim != 3 or grad_out.ndim != 4:
        raise ValueError(
            "ssd_single_rank1_dense_output_backward_kernel_profile expects "
    ...
    _require_supported_md(M, D, where="ssd_single_rank1_dense_output_backward_kernel_profile")
    _require_chunk_size_multiple_of_16(C_CHUNK, where="ssd_single_rank1_dense_output_backward_kernel_profile")
    _require_nonpositive_log_alpha(log_alpha, where="ssd_single_rank1_dense_output_backward_kernel_profile")
    if not C.is_cuda:
        raise NotImplementedError("ssd_single_rank1_dense_output_backward_kernel_profile requires CUDA tensors.")
    _ensure_triton_allocator()
...
class SsdSingleRank1Triton(torch.autograd.Function):
    ...
    def forward(
        ...
        C_chunk, W_chunk, V_chunk, log_alpha_chunk, init_flat, B, N, H, M, D, NC_exec = _ssd_single_rank1_prepare_unchunked_inputs(
            C, W, V, log_alpha, initial_state,
            where="SsdSingleRank1Triton.forward",
        ...
        S_local_end = _ssd_single_rank1_chunk_end_state_forward_impl(W_chunk, V_chunk, log_alpha_chunk, 32)
        ...
        S0_chunk, S1_chunk = _ssd_single_rank1_prefix_scan_forward_impl(
        ...
        y_chunk, y_off_chunk = _ssd_single_rank1_dense_output_forward_impl(
    ...
    def backward(ctx, grad_y_chunk, grad_S1_chunk):
        ...
        log_alpha_per_chunk = torch.sum(log_alpha_chunk.float(), dim=-1)
        S0_chunk, _ = _ssd_single_rank1_prefix_scan_forward_impl(
        ...
        dC_chunk, dW_chunk, dV_chunk, dlog_alpha_chunk_p3, dS0_chunk = _ssd_single_rank1_dense_output_backward_impl(
            C_chunk,
            W_chunk,
            V_chunk,
            log_alpha_chunk,
        ...
        dS_local_end = _ssd_single_rank1_bwd_workspace_tensor(
            "phase2_dS_local_end",
            (BH, NC, MD),
            device=C_chunk.device,
            dtype=torch.float32,
        )
        d_log_alpha_per_chunk = _ssd_single_rank1_bwd_workspace_tensor(
            "phase2_dlog_per_chunk",
            (BH, NC),
            device=C_chunk.device,
            dtype=torch.float32,
        )
        d_init_f32 = _ssd_single_rank1_bwd_workspace_tensor(
            "phase2_dinit",
            (BH, MD),
            device=C_chunk.device,
            dtype=torch.float32,
        ...
        ssd_single_rank1_prefix_scan_bwd_dense_kernel[grid_bwd](
            grad_chunk_start_f,
            grad_final_state_f,
            S0_chunk_f,
            log_alpha_per_chunk_f,
        ...
        grad_s_md = dS_local_end.view(BH, NC, M, D)
        ...
        ssd_single_rank1_chunk_end_state_bwd_dw_chunk_kernel[grid_dw](
        ...
        b_partials = _ssd_single_rank1_bwd_workspace_tensor(
            "phase1_b_partials",
        ...
        ssd_single_rank1_chunk_end_state_bwd_dv_b_chunk_kernel[grid_dv](
        ...
        ssd_single_rank1_chunk_end_state_bwd_dr_kernel[grid_dr](
        ...
        dW_chunk.add_(d_W_p1)
        dV_chunk.add_(d_V_p1)
        ...
        dlog_alpha_chunk_p3.add_(d_log_alpha_p1)
        dlog_alpha_chunk_p3.add_(d_log_alpha_per_chunk.unsqueeze(-1).to(log_alpha_chunk.dtype))
        dlog_alpha_chunk = dlog_alpha_chunk_p3

        # ---- Restore outer layout ----
        # Convert chunked [BH,NC,C,*] grads back to user-facing [B,N,H,*].
        dC = _ssd_single_rank1_restore_grad_layout(dC_chunk, B=B, N=N, H=H, C=CHUNK_SIZE)
        dW = _ssd_single_rank1_restore_grad_layout(dW_chunk, B=B, N=N, H=H, C=CHUNK_SIZE)
        dV = _ssd_single_rank1_restore_grad_layout(dV_chunk, B=B, N=N, H=H, C=CHUNK_SIZE)
...
def _ssd_single_rank1_restore_grad_layout(
...
def ssd_single_rank1_triton(
    ...
    N = C.shape[1]
    H = C.shape[2]
    return _ssd_single_rank1_restore_output_layout(y_chunk, S1_chunk, B=B, N=N, H=H, C=CHUNK_SIZE)
...
__all__ = [
    "ssd_single_rank1_pytorch",
    "ssd_single_rank1_triton",
]
