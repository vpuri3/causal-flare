...
def _restore_grad_layout(
    ...
    *,
    ...
    D = grad_chunk.shape[-1]
    return (
        grad_chunk.reshape(B, H, grad_chunk.shape[1], C, D)
        .permute(0, 2, 3, 1, 4)
        .reshape(B, grad_chunk.shape[1] * C, H, D)
    )[:, :N]
...
# --------------------------------------------------------------------------------------------------
@triton.jit
def ssd_single_rank1_phase1_fwd_kernel(
    V_ptr,
    log_alpha_ptr,
    out_s_end_ptr,
    bh_size,
    ...
    out_desc = tl.make_tensor_descriptor(
        out_s_end_ptr,
        shape=[bh_size, nc_size, d_size],
        strides=[stride_out_bh, stride_out_nc, stride_out_d],
    ...
    out_desc.store([BH_IDX, NC_IDX, d_start], tl.reshape(s_end, (1, 1, BLOCK_D)))


def _phase1_forward_impl(V: torch.Tensor, log_alpha: torch.Tensor) -> torch.Tensor:
    if V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
            "_phase1_forward_impl expects V=[BH,NC,C,D], log_alpha=[BH,NC,C]. "
            f"Got V={tuple(V.shape)}, log_alpha={tuple(log_alpha.shape)}."
        )
    BH, NC, C, D = V.shape
    if log_alpha.shape != (BH, NC, C):
        raise ValueError(f"log_alpha must be [BH,NC,C]={BH, NC, C}; got {tuple(log_alpha.shape)}.")
    _require_chunk_size_multiple_of_16(C, where="_phase1_forward_impl")
    if D not in _SUPPORTED_D_VALUES:
        raise NotImplementedError(f"_phase1_forward_impl requires D in {sorted(_SUPPORTED_D_VALUES)}; got D={D}.")
    if not V.is_cuda:
        raise NotImplementedError("_phase1_forward_impl requires CUDA tensors.")
    _ensure_triton_allocator()

    BLOCK_D = _select_largest_block_size(D, _SUPPORTED_BLOCK_X_VALUES, where="_phase1_forward_impl", label="D")
    out = torch.empty((BH, NC, D), device=V.device, dtype=torch.float32)
    grid = (BH * NC, D // BLOCK_D)
    ssd_single_rank1_phase1_fwd_kernel[grid](
        V,
        log_alpha,
        out,
        BH,
...
# --------------------------------------------------------------------------------------------------
@triton.jit
def ssd_single_rank1_phase3_fwd_kernel(
    V_ptr,
    log_alpha_ptr,
    S0_ptr,
    out_y_ptr,
    ...
    s0_desc = tl.make_tensor_descriptor(
        S0_ptr,
        shape=[bh_size, nc_size, d_size],
        strides=[stride_s0_bh, stride_s0_nc, stride_s0_d],
        block_shape=[1, 1, BLOCK_D],
    )
    out_desc = tl.make_tensor_descriptor(
        out_y_ptr,
    ...
    S0_tile = tl.reshape(s0_desc.load([BH_IDX, NC_IDX, d_start]), (BLOCK_D,)).to(tl.float32)
...
def _phase3_forward_impl(
    ...
    *,
    ...
    if V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError(
            "_phase3_forward_impl expects V=[BH,NC,C,D], log_alpha=[BH,NC,C]. "
            f"Got V={tuple(V.shape)}, log_alpha={tuple(log_alpha.shape)}."
        )
    BH, NC, C, D = V.shape
    if log_alpha.shape != (BH, NC, C):
    ...
    if S0.ndim == 2 and S0.shape == (BH, D):
        S0_vec = S0[:, None, :].expand(BH, NC, D)
    elif S0.ndim == 3 and S0.shape == (BH, NC, D):
        S0_vec = S0
    else:
        raise ValueError(f"S0 must be [BH,D] or [BH,NC,D]. Got {tuple(S0.shape)}.")
    _require_chunk_size_multiple_of_16(C, where="_phase3_forward_impl")
    if D not in _SUPPORTED_D_VALUES:
        raise NotImplementedError(f"_phase3_forward_impl requires D in {sorted(_SUPPORTED_D_VALUES)}; got D={D}.")
    if not V.is_cuda:
        raise NotImplementedError("_phase3_forward_impl requires CUDA tensors.")
    _ensure_triton_allocator()

    BLOCK_D = _select_largest_block_size(D, _SUPPORTED_BLOCK_X_VALUES, where="_phase3_forward_impl", label="D")
...
@triton.jit
def ssd_single_rank1_phase3_bwd_kernel(
    V_ptr,
    log_alpha_ptr,
    grad_y_ptr,
    S0_ptr,
    ...
    ds0_desc = tl.make_tensor_descriptor(
        out_dS0_ptr,
        shape=[bh_size, nc_size, d_size],
        strides=[stride_ds0_bh, stride_ds0_nc, stride_ds0_d],
        block_shape=[1, 1, BLOCK_D],
    )
    ...
    for d_blk in tl.static_range(0, triton.cdiv(D_STATIC, BLOCK_D)):
        ...
        ds0_desc.store([BH_IDX, NC_IDX, d_start], tl.reshape(ds0_tile, (1, 1, BLOCK_D)))
    ...
    dlog_desc.store([BH_IDX, NC_IDX, 0], tl.reshape(dlog_diag + dlog_off, (1, 1, BLOCK_C)))


def _phase3_backward_impl(
    V: torch.Tensor,
    log_alpha: torch.Tensor,
    grad_y: torch.Tensor,
    S0: torch.Tensor,
    *,
    ...
    if V.ndim != 4 or log_alpha.ndim != 3 or grad_y.ndim != 4:
        raise ValueError("_phase3_backward_impl expects V/log_alpha/grad_y with chunked rank-4/3/4 shapes.")
    BH, NC, C, D = V.shape
    if log_alpha.shape != (BH, NC, C):
        raise ValueError("log_alpha shape mismatch.")
    if grad_y.shape != (BH, NC, C, D):
        raise ValueError("grad_y shape mismatch.")
    if S0.ndim == 2 and S0.shape == (BH, D):
        S0_vec = S0[:, None, :].expand(BH, NC, D)
    elif S0.ndim == 3 and S0.shape == (BH, NC, D):
        S0_vec = S0
    else:
        raise ValueError("S0 must be [BH,D] or [BH,NC,D].")
    _require_chunk_size_multiple_of_16(C, where="_phase3_backward_impl")
    if D not in _SUPPORTED_D_VALUES:
        raise NotImplementedError(f"_phase3_backward_impl requires D in {sorted(_SUPPORTED_D_VALUES)}; got D={D}.")
    if not V.is_cuda:
        raise NotImplementedError("_phase3_backward_impl requires CUDA tensors.")
    _ensure_triton_allocator()

    BLOCK_D = _select_largest_block_size(D, _SUPPORTED_BLOCK_X_VALUES, where="_phase3_backward_impl", label="D")
    ...
    dS0_out = dS0 if return_s0_grad_fp32 else dS0.to(V.dtype)
    return dV, dlog.to(log_alpha.dtype), dS0_out
...
# --------------------------------------------------------------------------------------------------
@triton.jit
def ssd_single_rank1_phase1_bwd_dv_b_kernel(
    grad_s_ptr,
    V_ptr,
    log_alpha_ptr,
    out_dV_ptr,
    ...
    gs_desc = tl.make_tensor_descriptor(
        grad_s_ptr,
        shape=[bh_size, nc_size, d_size],
        strides=[stride_gs_bh, stride_gs_nc, stride_gs_d],
        block_shape=[1, 1, BLOCK_D],
    )
    v_desc = tl.make_tensor_descriptor(
        V_ptr,
    ...
    bp_desc = tl.make_tensor_descriptor(
        out_b_partials_ptr,
        shape=[bh_size, nc_size, n_d_tiles, c_size],
        strides=[stride_bp_bh, stride_bp_nc, stride_bp_dt, stride_bp_c],
    ...
    g = tl.reshape(gs_desc.load([BH_IDX, NC_IDX, d_start]), (BLOCK_D,)).to(tl.float32)
    V_in = tl.reshape(v_desc.load([BH_IDX, NC_IDX, 0, d_start]), (BLOCK_C, BLOCK_D))
    V_f = V_in.to(tl.float32)

    dV_tile = factors[:, None] * g[None, :]
    dv_desc.store([BH_IDX, NC_IDX, 0, d_start], tl.reshape(dV_tile.to(V_in.dtype), (1, 1, BLOCK_C, BLOCK_D)))

    b_partial = factors * tl.sum(V_f * g[None, :], axis=1)
    bp_desc.store([BH_IDX, NC_IDX, pid_d_tile, 0], tl.reshape(b_partial, (1, 1, 1, BLOCK_C)))


@triton.jit
def ssd_single_rank1_phase1_bwd_dlog_kernel(
    b_vals_ptr,
    out_dlog_ptr,
    bh_size,
    nc_size,
    ...
    b = tl.reshape(b_desc.load([BH_IDX, NC_IDX, 0]), (BLOCK_C,)).to(tl.float32)
    prefix = tl.cumsum(b, axis=0)
    out_desc.store([BH_IDX, NC_IDX, 0], tl.reshape(prefix - b, (1, 1, BLOCK_C)))
...
def _phase1_backward_impl(
    ...
    if grad_s.ndim != 3 or V.ndim != 4 or log_alpha.ndim != 3:
        raise ValueError("_phase1_backward_impl expects grad_s=[BH,NC,D], V=[BH,NC,C,D], log_alpha=[BH,NC,C].")
    BH, NC, C, D = V.shape
    if grad_s.shape != (BH, NC, D):
        raise ValueError(f"grad_s must be [BH,NC,D]={BH, NC, D}; got {tuple(grad_s.shape)}.")
    if log_alpha.shape != (BH, NC, C):
        raise ValueError(f"log_alpha must be [BH,NC,C]={BH, NC, C}; got {tuple(log_alpha.shape)}.")
    _require_chunk_size_multiple_of_16(C, where="_phase1_backward_impl")
    if D not in _SUPPORTED_D_VALUES:
        raise NotImplementedError(f"_phase1_backward_impl requires D in {sorted(_SUPPORTED_D_VALUES)}; got D={D}.")
    if not V.is_cuda:
        raise NotImplementedError("_phase1_backward_impl requires CUDA tensors.")
    _ensure_triton_allocator()

    BLOCK_D = _select_largest_block_size(D, _SUPPORTED_BLOCK_X_VALUES, where="_phase1_backward_impl", label="D")
    n_d_tiles = D // BLOCK_D
    dV = torch.empty_like(V)
    ...
    ssd_single_rank1_phase1_bwd_dv_b_kernel[grid_dv](
        grad_s,
        V,
        log_alpha,
        dV,
    ...
    return dV, dlog.to(log_alpha.dtype)
...
class SsdSingleRank1Triton(torch.autograd.Function):
    ...
    def backward(ctx, grad_y_chunk, grad_S1_chunk):
        ...
        # Phase-3 backward.
        dV_p3, dlog_p3, dS0 = _phase3_backward_impl(
            V_chunk,
            log_alpha_chunk,
            grad_y_chunk,
            S0_chunk,
        ...
        grad_final = (
            ...
            else torch.zeros((BH, D), device=V_chunk.device, dtype=phase2_compute_dtype)
        )
