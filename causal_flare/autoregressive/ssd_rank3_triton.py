...
def _ssd_rank3_collect_rank_terms_unchunked(
    C: torch.Tensor,
    ...
    *,
    ...
    for rank, Wk, Vk in ((2, W2, V2), (3, W3, V3)):
...
def _ssd_rank3_collect_rank_terms_chunked(
    W1: torch.Tensor | None,
    V1: torch.Tensor | None,
    W2: torch.Tensor | None = None,
    V2: torch.Tensor | None = None,
...
def ssd_rank3_chunk_end_state_reference(
    W1: torch.Tensor,
    V1: torch.Tensor,
    log_alpha: torch.Tensor,
    W2: torch.Tensor | None = None,
    ...
    """Phase-1 rank-3 reference: local chunk end-state for up to three rank-1 writes."""
    terms, BH, NC, C_CHUNK, M, D = _ssd_rank3_collect_rank_terms_chunked(
        W1, V1, W2, V2, W3, V3, where="ssd_rank3_chunk_end_state_reference"
    )
    if log_alpha.ndim != 3 or log_alpha.shape != (BH, NC, C_CHUNK):
        raise ValueError(
            "ssd_rank3_chunk_end_state_reference expects log_alpha=[BH,NC,C]. "
    ...
    if C_CHUNK == 0:
        raise NotImplementedError("ssd_rank3_chunk_end_state_reference does not support C==0.")
    _require_supported_md(M, D, where="ssd_rank3_chunk_end_state_reference")
    _require_nonpositive_log_alpha(log_alpha, where="ssd_rank3_chunk_end_state_reference")
...
def ssd_rank3_dense_output_reference(
    C: torch.Tensor,
    W1: torch.Tensor,
    V1: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    """Phase-3 rank-3 dense reference: sums up to three rank-1 write terms."""
    terms, BH, NC, C_CHUNK, M, D = _ssd_rank3_collect_rank_terms_chunked(
        W1, V1, W2, V2, W3, V3, where="ssd_rank3_dense_output_reference"
    )
    ...
    if C_CHUNK == 0:
        raise NotImplementedError("ssd_rank3_dense_output_reference does not support C==0.")
    _require_supported_md(M, D, where="ssd_rank3_dense_output_reference")
    _require_nonpositive_log_alpha(log_alpha, where="ssd_rank3_dense_output_reference")
...
def ssd_rank3_pytorch(
    ...
    """SSD rank-3 PyTorch reference/oracle entrypoint with up to three write terms."""
    terms, B, N, H, M, D = _ssd_rank3_collect_rank_terms_unchunked(
        C, W1, V1, W2, V2, W3, V3, where="ssd_rank3_pytorch"
    )
    if log_alpha.ndim != 3 or log_alpha.shape != (B, N, H):
        raise ValueError(f"ssd_rank3_pytorch: log_alpha must be [B,N,H]={B, N, H}; got {tuple(log_alpha.shape)}.")
    if log_alpha.device != C.device or log_alpha.dtype != C.dtype:
        raise ValueError("ssd_rank3_pytorch: log_alpha must be on the same device and dtype as C.")
    ...
    S_local_end = ssd_rank3_chunk_end_state_reference(
        w_chunk_bh[0],
        v_chunk_bh[0],
        log_alpha_chunk_bh,
        W2=W2_chunk,
    ...
    y_chunk = ssd_rank3_dense_output_reference(
        C_chunk_bh,
        w_chunk_bh[0],
        v_chunk_bh[0],
        log_alpha_chunk_bh,
...
def ssd_rank3_token_loop_oracle(
    C: torch.Tensor,
    W1: torch.Tensor,
    V1: torch.Tensor,
    log_alpha: torch.Tensor,
    ...
    """Unchunked token-loop oracle for rank-3 recurrence with optional extra write terms."""
    terms, B, N, H, M, D = _ssd_rank3_collect_rank_terms_unchunked(
        C, W1, V1, W2, V2, W3, V3, where="ssd_rank3_token_loop_oracle"
    )
    if log_alpha.ndim != 3 or log_alpha.shape != (B, N, H):
        raise ValueError(
            "ssd_rank3_token_loop_oracle expects log_alpha=[B,N,H]. "
            f"Got log_alpha={tuple(log_alpha.shape)} with expected {(B, N, H)}."
        )
    if log_alpha.device != C.device or log_alpha.dtype != C.dtype:
        raise ValueError("ssd_rank3_token_loop_oracle: log_alpha must share device/dtype with C.")
    if N == 0:
        raise NotImplementedError("ssd_rank3_token_loop_oracle does not support N==0.")
    _require_supported_md(M, D, where="ssd_rank3_token_loop_oracle")
    _require_nonpositive_log_alpha(log_alpha, where="ssd_rank3_token_loop_oracle")
...
def ssd_rank3_triton(
    ...
    terms, B, N, H, M, D = _ssd_rank3_collect_rank_terms_unchunked(
        ...
        W3,
        V3,
        where="ssd_rank3_triton",
    )
    if log_alpha.ndim != 3 or log_alpha.shape != (B, N, H):
        raise ValueError(
            "ssd_rank3_triton expects log_alpha=[B,N,H]. "
            f"Got log_alpha={tuple(log_alpha.shape)} with expected {(B, N, H)}."
        )
    if log_alpha.device != C.device:
        raise ValueError("ssd_rank3_triton: log_alpha must be on the same device as C/W/V.")
...
__all__ = [
    "ssd_rank3_chunk_end_state_reference",
    "ssd_rank3_dense_output_reference",
    "ssd_rank3_pytorch",
    "ssd_rank3_triton",
    "ssd_rank3_token_loop_oracle",
    "ssd_rank1_pytorch",
