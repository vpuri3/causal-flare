...
def _chunk_scan_fwd_kernel(
    ...
    if IS_TRITON_22 or pid_c > -1:
        ...
        if BLOCK_SIZE_DSTATE <= 128:
            ...
            acc = tl.dot(C, prev_states, input_precision="ieee") * scale_m[:, None]
        else:
            for k in range(0, dstate, BLOCK_SIZE_K):
                ...
                prev_states = prev_states.to(C_ptr.dtype.element_ty)
                acc += tl.dot(C, prev_states, input_precision="ieee")
                C_ptrs += BLOCK_SIZE_K
    ...
    for k in range(0, K_MAX, BLOCK_SIZE_K):
        ...
        acc += tl.dot(cb, x, input_precision="ieee")
...
def _chunk_scan_bwd_dstates_kernel(
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        ...
        acc += tl.dot(dout, c, input_precision="ieee")
...
@triton.autotune(
    configs=autotune_configs([
        ...
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64}, num_stages=3, num_warps=1, pre_hook=init_to_zero(["ddA_cumsum_ptr"])),
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 64}, num_stages=3, num_warps=1, pre_hook=init_to_zero(["ddA_cumsum_ptr"])),
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 32}, num_stages=3, num_warps=1, pre_hook=init_to_zero(["ddA_cumsum_ptr"])),
    ]),
...
def _chunk_scan_bwd_dc_kernel(
    ...
    for h in range(nheads_iter):
        ...
        dc = tl.dot(dout, prev_states, input_precision="ieee")
...
@triton.autotune(
    configs=autotune_configs([
        ...
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64}, num_stages=3, num_warps=1),
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 64}, num_stages=3, num_warps=1),
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 32}, num_stages=3, num_warps=1),
...
def _chunk_scan_bwd_dcb_kernel(
    ...
    for h in range(nheads_iter):
        ...
        dcb = tl.dot(dout, x, input_precision="ieee")
...
class ChunkScanFn(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        dC, ddA_cumsum_prev = _chunk_scan_bwd_dC(prev_states, dA_cumsum, dout, C=C, ngroups=ngroups)
        dC = dC.to(C.dtype)
        ...
        if z is not None:
            ...
            ddA_cumsum = _chunk_scan_bwd_ddAcs_stable(x, dt, dA_cumsum, dout, CB) + ddA_prev
            dD = None
