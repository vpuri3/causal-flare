...
def _chunk_state_fwd_kernel(
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        ...
        if not HAS_SEQ_IDX:
            scale = tl.exp(tl.minimum((dA_cs_last - dA_cs_k), 0.0))
        else:
            scale = tl.where((seq_idx_last >= 0) & (seq_idx_k == seq_idx_last), tl.exp(tl.minimum((dA_cs_last - dA_cs_k), 0.0)), 0.0)
        dt_k = tl.load(dt_ptrs, mask=offs_k < chunk_size_limit - k, other=0.0).to(tl.float32)
        scale *= dt_k
        b *= scale[:, None]
        b = b.to(x_ptr.dtype.element_ty)
        acc += tl.dot(x, b, input_precision="ieee")
...
def _chunk_state_bwd_dx_kernel(
    ...
    # Meta-parameters
    DETERMINISTIC_REDUCTION: tl.constexpr,
    STORE_DDT: tl.constexpr,
    USE_FULL_DSTATE_TILE: tl.constexpr,
    BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr,
    ...
    chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)
    # The single-shot full-dstate tile is faster when it fits comfortably, but
    # on larger dstate it can become shared-memory limited. Keep the fast path
    # explicit and let the wrapper choose when to fall back to the looped path.
    offs_k = tl.arange(0, BLOCK_SIZE_DSTATE if USE_FULL_DSTATE_TILE else BLOCK_SIZE_K)
    ...
    dt_m = tl.load(dt_ptr + offs_m * stride_dt_csize, mask=offs_m < chunk_size, other=0.0).to(tl.float32)
    ddA_cs = -(ddt * dt_m)
...
def _chunk_state_bwd_db_kernel(
    ...
    for h in range(nheads_iter):
        ...
        db = tl.dot(x, dstates, input_precision="ieee")
        dA_cs_last = tl.load(dA_cumsum_ptr + (chunk_size - 1) * stride_dA_cs_csize).to(tl.float32)
        dA_cs_m = tl.load(dA_cumsum_ptrs, mask=offs_m < chunk_size, other=0.0).to(tl.float32)
        if not HAS_SEQ_IDX:
...
def _chunk_state_bwd_dx(B, x, dt, dA_cumsum, dstates, dx=None, return_ddt=True):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    _, _, ngroups, dstate = B.shape
    assert nheads % ngroups == 0
    ...
    if return_ddt:
        ...
        # The Triton autotune pre-hook always zeroes `ddt_ptr`, even when
        # STORE_DDT=False. Do not alias it to `dx`, or we clear the full output
        # buffer before overwriting it. A zero-sized tensor keeps the pre-hook
        # cheap while remaining unused in the kernel body.
        ddt, stride_ddt_tile = dx, 0
    ...
    # The one-shot dstate tile spills badly for dstate=128 on Hopper. Keep the
    # full tile for smaller state sizes, but cap the working tile at 64 so the
    # kernel can use the looped path on larger states.
    block_size_dstate = max(min(triton.next_power_of_2(dstate), 64), 16)
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_bwd_dx_kernel[grid_dx](
            ...
            DETERMINISTIC_REDUCTION=deterministic,
            STORE_DDT=return_ddt,
            USE_FULL_DSTATE_TILE=block_size_dstate >= dstate,
            BLOCK_SIZE_DSTATE=block_size_dstate,
        )
    ddt = finalize_tile_workspace(ddt, deterministic) if return_ddt else None
...
class ChunkStateFn(torch.autograd.Function):
    ...
    def backward(ctx, dstates):
        ...
        dx, ddt, _ddA_cumsum_unused = _chunk_state_bwd_dx(B, x, dt, dA_cumsum, dstates)
        dB, ddA_cumsum = _chunk_state_bwd_db(x, dt, dA_cumsum, dstates, B=B, ngroups=ngroups)
        dB = dB.to(B.dtype)
