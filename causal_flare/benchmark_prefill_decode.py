...
- prefill: `flare_prefill_triton`, `flare_prefill_pytorch`
- decode: `flare_decode_triton`, `flare_decode_pytorch`
...
def _build_prefill_fn(
    *,
    ...
    if provider == "pytorch":
        ...
        def run() -> None:
            flare_prefill_pytorch(Q=q, K=k, V=v, scale=scale)
...
def _build_decode_fn(
    *,
    ...
    if provider == "triton":
        ...
        _, base_state = flare_prefill_pytorch(
            Q=q,
            K=k_prompt,
            V=v_prompt,
            scale=scale,
...
def run_benchmark(args: argparse.Namespace, output_dir: Path) -> pd.DataFrame:
    ...
    include_pytorch = not args.skip_pytorch
    cases = build_cases(
...
def _curve_label(provider: str, m: int) -> str:
    provider_name = "Triton" if provider == "triton" else "PyTorch"
    return f"{provider_name} (M={m})"
...
def plot_summary(df: pd.DataFrame, png_path: Path, pdf_path: Path, title: str) -> None:
    ...
    provider_colors = {"triton": "#1f77b4", "pytorch": "#ff7f0e"}
    ...
    for ax, (mode, y_col, y_lo_col, y_hi_col, y_label) in zip(axes, mode_specs):
        ...
        groups = []
        for provider in ("triton", "pytorch"):
            sub_provider = mode_df[mode_df["provider"] == provider]
            if sub_provider.empty:
                continue
...
def main() -> None:
    ...
    if args.skip_triton and args.skip_pytorch:
        raise ValueError("Both --skip-triton and --skip-pytorch are set; nothing to run.")
