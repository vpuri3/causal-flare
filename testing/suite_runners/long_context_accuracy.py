...
def _long_context_accuracy_suite(*, shard_index: int | None = None, num_shards: int | None = None):
    ...
    env_num_shards = int(os.environ.get("FLARE_LONGCTX_NUM_SHARDS", "1"))
    env_shard_index = int(os.environ.get("FLARE_LONGCTX_SHARD_INDEX", "0"))
    if num_shards is None:
        num_shards = env_num_shards
    if shard_index is None:
        shard_index = env_shard_index
    if num_shards <= 0:
        raise ValueError(f"FLARE_LONGCTX_NUM_SHARDS must be positive, got {num_shards}.")
    if shard_index < 0 or shard_index >= num_shards:
        raise ValueError(f"FLARE_LONGCTX_SHARD_INDEX must be in [0, {num_shards}), got {shard_index}.")
    failures: list[str] = []
    case_index = 0
    selected_cases = 0
    ...
    print(f"[FLARE LONGCTX] selected_cases={selected_cases} shard={shard_index + 1}/{num_shards}")
...
def _long_context_accuracy_suite_shard(shard_index: int, num_shards: int) -> None:
    _long_context_accuracy_suite(shard_index=shard_index, num_shards=num_shards)
