"""Small full-autotune correctness coverage for representative launch configs."""
...
def _autotune_launch_coverage_suite(*, shard_index: int | None = None, num_shards: int | None = None) -> None:
    ...
    precisions = _precision_modes_from_env("FLARE_AUTOTUNE_COVERAGE_PRECISIONS")
    ...
    print("=" * 100)
    print("[FLARE AUTOTUNE] Full-autotune launch coverage")
    print("=" * 100)
    print(
    ...
    with _temp_env_var("FLARE_TEST_REDUCE_AUTOTUNE", ""):
            ...
            with _temp_env_var("FLARE_INPUT_PRECISION", precision):
                ...
                if strict:
                    ...
                    if precision == "ieee":
                        _maybe_record_failure(
                            failures,
                            y_err["autotune_fwd_mean_abs_err"] > fwd_mean_abs_max,
                            f"Autotune coverage fwd mean_abs too high: {y_err['autotune_fwd_mean_abs_err']:.3e} > {fwd_mean_abs_max:.3e}",
                        )
                        _maybe_record_failure(
                            failures,
                            y_err["autotune_fwd_max_abs_err"] > fwd_max_abs_max,
                            f"Autotune coverage fwd max_abs too high: {y_err['autotune_fwd_max_abs_err']:.3e} > {fwd_max_abs_max:.3e}",
                        )
                    else:
