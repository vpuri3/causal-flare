...
def _parse_args():
    parser = argparse.ArgumentParser(description="Benchmark the torch Block-Causal FLARE prototype.")
...
def main():
    ...
    torch.manual_seed(0)
    q = torch.randn((args.num_heads, args.num_latents, args.head_dim), device=device, dtype=dtype)
    ...
    results = benchmark_block_causal_torch(
        q,
        k,
        v,
    ...
    print("Block-Causal FLARE benchmark")
    print(f"shape: B={args.batch_size} N={args.num_tokens} H={args.num_heads} M={args.num_latents} D={args.head_dim}")
    print(f"config: block_size={args.block_size} chunk_size={args.chunk_size} dtype={args.dtype} device={args.device}")
